...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'register') {
      ...
      try { await create(COLLECTIONS.CART, { userId: user.id }) } catch {}
      try { await create(COLLECTIONS.WISHLIST, { userId: user.id }) } catch {}
    ...
    if (action === 'login') {
      ...
      if (!user && email === ADMIN_EMAIL) {
        try {
          ...
          await create(COLLECTIONS.CART, { userId: user.id })
          await create(COLLECTIONS.WISHLIST, { userId: user.id })
